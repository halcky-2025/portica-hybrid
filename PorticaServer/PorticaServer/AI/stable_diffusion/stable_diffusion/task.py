    ...
    while TRUE:
        try:
            ...
            if len(o.get('prompt', '')) == 0:
                time.sleep(1)
                continue
            typo = o.get('type', 'task')
            secret = o.get('secret', '')
            options = o.get('options', None)
            if options:
                model2 = options.get('model', 'sd1.4')
                if model2 == "sd1.4":
                ...
                par.scale = float(options.get('scale', 7.5))
                par.seed = int(options.get('seed', par.seed))
                par.plms = options.get('plms', None)
                par.ddim_steps = int(options.get('ddim_steps', 50))
                par.sample = options.get('sample', None)
                if par.sample:
                    par.translation_x = int(options.get('translation_x', 0))
                    par.translation_y = int(options.get('translation_y', 0))
                    par.zoom = float(options.get('zoom', 1.0))
                    par.angle = float(options.get('angle', 0))
                    par.noise = float(options.get('noise', 0))
                    par.contrast = float(options.get('contrast', 1))
                ...
                if init_img:
                    ...
                    if len(init_img_content) > 0:
                        init_img_content = base64.b64decode(init_img_content)
            ...
            if par.init_img is None:
                        ...
                        with model.ema_scope():
                            ...
                            for n in trange(par.n_iter, desc="Sampling"):
                                for prompts in tqdm(data, desc="data"):
                                    ...
                                    samples_ddim, _ = sampler.sample(S=par.ddim_steps,
                                                                        conditioning=c,
                                                                        batch_size=opt.n_samples,
                                                                        shape=shape,
                                                                        verbose=False,
                ...
                grid_count = len(os.listdir(outpath)) - 1
                if par.sample:
                    im = cv2.imread(par.init_img)
                    im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
                    h, w, c = im.shape
                    prev_sample = sample_from_cv2(im)
                    im = sample_to_cv2(prev_sample)
                    xform = make_xform_2d(w, h, par.translation_x, par.translation_y, par.angle, par.zoom)
                    prev_img = cv2.warpPerspective(im, xform, (im.shape[1], im.shape[0]), borderMode=cv2.BORDER_WRAP)

                    # apply scaling
                    contrast_sample = prev_img * par.contrast
                    # apply frame noising
                    noised_sample = add_noise(sample_from_cv2(contrast_sample), par.noise)
                    init_sample = noised_sample.to(device)
                        
                    init_latent = model.get_first_stage_encoding(
                        model.encode_first_stage(init_sample)
                    )
                else:
                ...
                sampler.make_schedule(ddim_num_steps=par.ddim_steps, ddim_eta=opt.ddim_eta, verbose=False)

                assert 0. <= par.strength <= 1., 'can only work with strength in [0.0, 1.0]'
                t_enc = int(par.strength * par.ddim_steps)
