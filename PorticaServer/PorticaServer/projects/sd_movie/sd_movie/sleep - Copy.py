    ...
    while TRUE:
        try:
            ...
            if save_path is None:
                ...
                r = http.request('POST', 'https://hal-tea.club/order', fields={
                    'secret': seins,
                    'error': error,
                    })
                print(r.data.decode('utf-8'))
                o = json.loads(r.data.decode('utf-8'))
                par.prompt = o.get('prompt', '')
                error = ''
                if len(par.prompt) == 0:
                   time.sleep(1)
                   continue
                model = o.get('model', 'sd1.4')
                if model == 'wd1.2':
                    par.ckpt = 'models/ldm/wd-v1-2.ckpt'
                secret = o.get('secret', '')
                par.W = o.get('W', 512)
                par.H = o.get('H', 512)
                init_img = o.get('init_img', '')
                if len(init_img) > 0:
                   init_img = base64.b64decode(init_img)
                   file_name = 'a' + o.get('ex', '.png')
                   with open(file_name, 'wb') as f:
                       f.write(init_img)
                       par.init_img = file_name
                       par.scale = 5.0
                       par.n_iter = 1
                par.strength = o.get('strength', 0.75)
            else:
                ...
                r = http.request('POST', 'https://hal-tea.club/order', fields={
                    'secret': seins,
                    'error': error,
                    'upload_img': ('update.png', upload_img, 'image/png')
                    })
                print(r.data.decode('utf-8'))
                o = json.loads(r.data.decode('utf-8'))
                par.prompt = o.get('prompt', '')
